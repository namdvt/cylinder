...
class DOTADataset(CustomDataset):
    ...
    def evaluate(self,
        ...
        # TODO: eval on cylinder
        top_annotations = []
        for ann in annotations:
            top_annotations.append(
                {'bboxes':ann['bboxes'][:,0:5],
                'labels':ann['labels']}
            )
        bottom_annotations = []
        for ann in annotations:
            bottom_annotations.append(
                {'bboxes':ann['bboxes'][:,6:11],
                'labels':ann['labels']}
            )
        ...
        if metric == 'mAP':
            ...
            top_results = [[r[0][:,0:6]] for r in results]          
            top_mean_ap, _ = eval_rbbox_map(
                top_results,
                top_annotations,
            ...
            bottom_results = [[r[0][:,6:12]] for r in results]
            bottom_mean_ap, _ = eval_rbbox_map(
                bottom_results,
                bottom_annotations,
                scale_ranges=scale_ranges,
                iou_thr=iou_thr,
            ...
            eval_results['mAP'] = (top_mean_ap + bottom_mean_ap) / 2
        else:
