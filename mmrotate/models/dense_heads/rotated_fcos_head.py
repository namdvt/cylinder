...
class RotatedFCOSHead(RotatedAnchorFreeHead):
    ...
    def _init_predictor(self):
        ...
        self.conv_cls = nn.Conv2d(
            self.feat_channels, self.cls_out_channels * 2, 3, padding=1)
        self.conv_reg = nn.Conv2d(self.feat_channels, 8, 3, padding=1)
        
    def _init_layers(self):
        """Initialize layers of the head."""
        super()._init_layers()
        self.conv_centerness = nn.Conv2d(self.feat_channels, 2, 3, padding=1)
        self.conv_angle = nn.Conv2d(self.feat_channels, 2, 3, padding=1)
    ...
    def loss(self,
        ...
        """
        top_cls_scores = [cls[:,0:1,:,:] for cls in cls_scores]
        top_bbox_preds = [bbox[:,0:4,:,:] for bbox in bbox_preds]
        top_angle_preds = [angle[:,0:1,:,:] for angle in angle_preds]
        top_centernesses = [center[:,0:1,:,:] for center in centernesses]
        top_gt_bboxes = [bbox[:,0:5] for bbox in gt_bboxes]
        
        bottom_cls_scores = [cls[:,1:2,:,:] for cls in cls_scores]
        bottom_bbox_preds = [bbox[:,4:8,:,:] for bbox in bbox_preds]
        bottom_angle_preds = [angle[:,1:2,:,:] for angle in angle_preds]
        bottom_centernesses = [center[:,1:2,:,:] for center in centernesses]
        bottom_gt_bboxes = [bbox[:,6:11] for bbox in gt_bboxes]
        
        top_loss = self.ellipse_loss(top_cls_scores,
             top_bbox_preds,
             top_angle_preds,
             top_centernesses,
             top_gt_bboxes,
        ...
        bottom_loss = self.ellipse_loss(bottom_cls_scores,
             bottom_bbox_preds,
             bottom_angle_preds,
             bottom_centernesses,
             bottom_gt_bboxes,
        ...
        loss = {}
        for key in top_loss:
            loss[key] = top_loss[key] + bottom_loss[key]
    ...
    def get_bboxes(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            # ------------top------------
            top_cls_score_list = [
                cls_scores[i][img_id][0:1].detach() for i in range(num_levels)
            ]
            top_bbox_pred_list = [
                bbox_preds[i][img_id][0:4].detach() for i in range(num_levels)
            ]
            top_angle_pred_list = [
                angle_preds[i][img_id][0:1].detach() for i in range(num_levels)
            ]
            top_centerness_pred_list = [
                centernesses[i][img_id][0:1].detach() for i in range(num_levels)
            ]
            # ------------bottom------------
            bottom_cls_score_list = [
                cls_scores[i][img_id][1:2].detach() for i in range(num_levels)
            ]
            bottom_bbox_pred_list = [
                bbox_preds[i][img_id][4:8].detach() for i in range(num_levels)
            ]
            bottom_angle_pred_list = [
                angle_preds[i][img_id][1:2].detach() for i in range(num_levels)
            ]
            bottom_centerness_pred_list = [
                centernesses[i][img_id][1:2].detach() for i in range(num_levels)
            ]
            img_shape = img_metas[img_id]['img_shape']
            scale_factor = img_metas[img_id]['scale_factor']
            
            top_det_bboxes = self._get_bboxes_single(top_cls_score_list,
                                                    top_bbox_pred_list,
                                                    top_angle_pred_list,
                                                    top_centerness_pred_list,
            ...
            bottom_det_bboxes = self._get_bboxes_single(bottom_cls_score_list,
                                                        bottom_bbox_pred_list,
                                                        bottom_angle_pred_list,
                                                        bottom_centerness_pred_list,
                                                        mlvl_points, img_shape,
                                                        scale_factor, cfg, rescale)
            # top_det_bboxes: (x, y, w, h, angle, conf)
            num_pred = min(top_det_bboxes[0].shape[0], bottom_det_bboxes[0].shape[0])
            cylinder_bboxes = torch.cat([top_det_bboxes[0][0:num_pred], bottom_det_bboxes[0][0:num_pred]], dim=1) # (x1, y1, w1, h1, angle1, conf1, x2, y2, w2, h2, angle2, conf2)
            result_list.append([cylinder_bboxes, bottom_det_bboxes[1][0:num_pred]])
        return result_list
